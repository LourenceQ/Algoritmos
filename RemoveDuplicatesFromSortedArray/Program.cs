...
namespace RemoveDuplicatesFromSortedArray
{
    class Program
    {
        ...
        private static int RemoveDuplicates(int[] nums)
        {
            ...
            var groupNums = nums.GroupBy(n => n)
                .Select(n => n.Key)
                .OrderBy(n => n).ToList();

            for(int x = 0; x < groupNums.Count(); x++)
            {
                nums[x] = groupNums[x];
            }

            return groupNums.Count();
            ...
            for (var i = 1; i < nums.Length; i++)
            {
                ...
                if (nums[i] != nums[i - 1])
                {
                    System.Console.WriteLine($"antes: " + c);
                    nums[c++] = nums[i - 1];
                    System.Console.WriteLine($"depois " + c);
                }
            ...
            // {
            //     System.Console.WriteLine($"{i+1}º = {nums[i]}");
            // }
