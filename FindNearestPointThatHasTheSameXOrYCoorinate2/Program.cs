...
{
    var res = points.Select((a, index) => (a, index))
        .Where(b => b.a[0] == x || b.a[1] == y).ToList();
    ...
    {
        var v = Math.Abs(e.a[0] - x) + Math.Abs(e.a[1] - y);
        if (v < r)
        {
            r = v;
            pos = e.index;
...
static int NearestValidPoint2(int x, object y, int[][] points)
{
    var nrst = -1;
