...
Then, some champagne is poured into the first glass at the top.  When the topmost glass is 
full, any excess liquid poured will fall equally to the glass immediately to the left and right of it. 
...
For example, after one cup of champagne is poured, the top most glass is full.  After two cups of
 champagne are poured, the two glasses on the second row are half full.  After three cups of champagne
  are poured, those two cups become full - there are 3 full glasses total now.  After four cups 
  of champagne are poured, the third row has the middle glass half full, and the two outside glasses
...
namespace ChampagneTower
{
    class Program
    {
        ...
        {
            var cRow = new double[] {poured};
            var pRow = cRow;

            for(int i=0; i<query_row; i++)
            {
                cRow = new double[pRow.Length+1];

                for(int j=0; j<pRow.Length; j++)
                {
                    if(pRow[j] > 1.0)
                    {
                        double spill = (pRow[j] -1.0) / 2.0;
                        cRow[j] += spill;
                        cRow[j+1] += spill;
                    }
                }
            ...
            return Math.Min(1.0, cRow[query_glass]);
        }
        ...
        for(int i = 0; i < query_row; i++){
            ...
            for(int j = 0; j < previousRow.Length; j++){
                ...
                if(previousRow[j] > 1.0){
                    double spillToTwoGlasses = (previousRow[j] - 1.0) / 2.0;
